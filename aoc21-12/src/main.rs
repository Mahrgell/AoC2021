...
fn main() {
    ...
    for l in contents.lines() {
        ...
        for (from, to) in [(words[0], words[1]), (words[1], words[0])] {
            connections.entry(String::from(from)).or_insert(Vec::new()).push(String::from(to));
        }
    }
    println!("Number of pathes: {}", number_of_pathes(&connections, false));
    println!("Number of pathes with one double visit: {}", number_of_pathes(&connections, true));
}